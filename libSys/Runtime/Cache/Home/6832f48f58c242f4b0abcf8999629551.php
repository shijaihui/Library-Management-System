...
if(pageId=='page4')
{
    ...
    $("#d2").css("display","inline");
       ...
       $("button.update").click(function(){
            var id = $(this).attr("id");
            var country = $("td#"+id+"-country").text();
            var tmin = $("td#"+id+"-tmin").text();
            var place = $("td#"+id+"-place").text();
            var sch = $("td#"+id+"-sch").text();
            var content = $("td#"+id+"-content").text();
        ...
        layer.confirm('编辑项目后，已通过的项目将重新提交审核', {
            btn: ['确定','取消'], //按钮
               ...
               layer.open({
                   ...
                   shade: 0.8,
                   area: ['700px', '480px'],
                   content: "generalTeacherOtherExcEdit.html" ,//iframe的url
                    success: function(layero,index)
                    {
                      var input_country = layer.getChildFrame('#country',index);
                      input_country.val(country);
                      var input_tmin = layer.getChildFrame('#tmin',index);
                      input_tmin.val(tmin);
                      var input_place = layer.getChildFrame('#place',index);
                      input_place.val(place);
                      var input_sch = layer.getChildFrame('#sch',index);
                      input_sch.val(sch);
                      var input_content = layer.getChildFrame('#content',index);
                      ...
                      input_id.click(function(){
                         ...
                         form1.ajaxSubmit({
                          url : "<?php echo U('Home/updateOtherExc');?>",
                          data: { data : id },
                          type : "post",
    ...
    $(document).ready(function(){
        $("button.add").click(function(){
            layer.open({
                ...
                shade: 0.8,
                area: ['700px', '480px'],
                content: 'generalTeacherOtherExcAdd.html', //iframe的url
                success: function(layero,index)
                {
                  var button = layer.getChildFrame('#btn',index);
                  button.click(function(){
                    var form = layer.getChildFrame('form#addotherexc',index);
                   
                      form.ajaxSubmit({
                          url : "<?php echo U('Home/insertOtherExc');?>",
                          type : "post",
                          enctype : "multipart/form-data",
                          dataType:'json',
                          success: function(data){
      ...
      $(document).ready(function(){
        ...
        $("button.delete").click(function(){
            ...
            layer.confirm('可点击“编辑”以再次编辑并重新提交审核,删除后将不可恢复，确定删除该项目吗？',{
                    ...
                    //删除确定的ajax代码应该放在这里
                     $.post("<?php echo U('Home/deleteOtherExc');?>",{
                        data  : id
                      },
...
<!-- 主体部分 -->
    <div class="main"style="padding-top: 50px;padding-left: 50px;">
        <div class="home">
            <h style="font-weight: bold;">国际交流</h>
        </div>
        <div class="teacher">
            <h name="xi"><?php echo ($otherExc_WAIT); ?>项待审核 <?php echo ($otherExc_YES); ?>项已通过 
            <?php echo ($otherExc_NO); ?>项未通过</h>    
        </div>
        <div class="hr0">
            <hr>
        </div>
       ...
       <button class="b active" href="#stay" id="bStay" data-toggle="tab" >待审核</button>
        ...
        <table>
            ...
            <tbody>
                    <?php if(is_array($otherExcWAIT)): $i = 0; $__LIST__ = $otherExcWAIT;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ta): $mod = ($i % 2 );++$i;?><tr id="<?php echo ($ta['num']); ?>-<?php echo ($ta['years']); ?>-WAIT">
        ...
        <table>
            ...
            <tbody>
                    <?php if(is_array($otherExcYES)): $i = 0; $__LIST__ = $otherExcYES;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ta): $mod = ($i % 2 );++$i;?><tr id="<?php echo ($ta['num']); ?>-<?php echo ($ta['years']); ?>-YES">
        ...
        <table>
            ...
            <tbody>
                 <?php if(is_array($otherExcNO)): $i = 0; $__LIST__ = $otherExcNO;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ta): $mod = ($i % 2 );++$i;?><tr id="<?php echo ($ta['num']); ?>-<?php echo ($ta['years']); ?>-NO">
